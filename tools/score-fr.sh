...
ORIG_SCORE=$(rust/scorer/target/release/scorer -t $TRACE -s $MODEL_SRC -d $MODEL_DST | grep 'FINAL ENERGY' | cut -f4 -d ' ')
if [ "$ORIG_SCORE" == "" ]; then
...
NEW_SCORE=$(rust/scorer/target/release/scorer -t $NEW_TRACE -s $MODEL_SRC -d $MODEL_DST | grep 'FINAL ENERGY' | cut -f4 -d ' ')
if [ "$NEW_SCORE" == "" ]; then
