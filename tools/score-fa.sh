...
ORIG_SCORE=$(rust/scorer/target/release/scorer -t $TRACE -d $MODEL | grep 'FINAL ENERGY' | cut -f4 -d ' ')
if [ "$ORIG_SCORE" == "" ]; then
...
NEW_SCORE=$(rust/scorer/target/release/scorer -t $NEW_TRACE -d $MODEL | grep 'FINAL ENERGY' | cut -f4 -d ' ')
if [ "$NEW_SCORE" == "" ]; then
